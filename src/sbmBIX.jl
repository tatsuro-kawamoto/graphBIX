function EM(Ntot,B,links,maxCrs,initial)
    ...
    function update_h()
        h = sum(PSI,1)*Crs/Ntot
    end
    
    function updatePSI(Crs,h,gr,PSIcav,nb,PSI)
        for i = 1:Ntot
            logPSIi = logunnormalizedMessage(i,Crs,h,gr,PSIcav,nb)
            PSI[i,:] = normalize_logprob(logPSIi)
        end
        return PSI
    end
    
    function logunnormalizedMessage(i,Crs,h,gr,PSIcav,nb)
        logmessages = sum([log(PSIcav[(s,i)]*Crs) for s in nb[i]],1)[1]
        for r = 1:B
    ...
    function BP()
        ...
        for k in randperm(size(links,1))
            (i,j) = tuple(links[k,:]...)
            logPSIi = logunnormalizedMessage(i,Crs,h,gr,PSIcav,nb)
            PSIcav[(i,j)] = logPSIi - log(PSIcav[(j,i)]*Crs) # this is actually log(PSIcav[(i,j)])
            PSIcav[(i,j)] = normalize_logprob(PSIcav[(i,j)])

            hprev = PSI[j,:]*Crs/Ntot
            grprev = PSI[j,:]/Ntot
            prev = PSI[j,:]/Ntot
            logPSIj = logunnormalizedMessage(j,Crs,h,gr,PSIcav,nb) # new PSI with new PSIcav
            PSI[j,:] = normalize_logprob(logPSIj)
            h += PSI[j,:]*Crs/Ntot - hprev
    ...
    function update_Crs()
        ...
        for ij = 1:Ktot
            ...
            ccrs = PSIcav[(i,j)]'*PSIcav[(j,i)].*Crs
    ...
    function freeenergy()
        sumlogZi = 0
        for i = 1:Ntot
            sumlogZi += logsumexp(logunnormalizedMessage(i,Crs,h,gr,PSIcav,nb))
        end
        ...
        for ij = 1:Ktot
            ...
            for s = 1:B
                for t=1:B
                    push!(logbiprob, log(Crs[s,t])+log(PSIcav[(i,j)][s])+log(PSIcav[(j,i)][t]))
                end
            end
            ...
            for s = 1:B
                for t=1:B
                    CVGPij += PSIcav[(i,j)][s]*(log(Crs[s,t])-log(Ntot))*PSIcav[(j,i)][t]
                end
            end
            CVGPs[cnt] = CVGPij
            ...
            for s = 1:B
                for t=1:B
                    CVGTij += PSIcav[(i,j)][s]*Crs[s,t]*(log(Crs[s,t])-log(Ntot))*PSIcav[(j,i)][t]/(Ntot*exp(logZijs[cnt])) # factor N cancels at Crs and Zij
                end
            end
            CVGTs[cnt] = CVGTij
            # MAP estimate
            ...
            #            CVMAP += MAPij*log(Crs[s,t])*MAPji    # this is wrong...
            CVMAPs[cnt] = log(Crs[s,t]) - log(Ntot)
            #################
        end
        avedegree = gr*Crs*gr'
    ...
    if fail == false        
        itrmax = 128
        BPconvthreshold = 0.001
        PSI = updatePSI(Crs,h,gr,PSIcav,nb,PSI)
        gr = update_gr(PSI)
        for itr = 1:itrmax
            (BPconv, PSI) = BP()
...
#        u = split(line, "\t")
    ...
    println("nodes & links updated... : N = $(Ntot) 2L = $(size(links,1))")
    if Ntot < Nthreshold
...
sbmBP
    ...
    for B = 2:Bmax
        ...
        for init = 1:length(initialconditions)
            ...
            while sm < samples
                Crsfail = false
                (gr,Crs,PSI,FE,CVBayes,CVGP,CVGT,CVMAP,varCVBayes,varCVGP,varCVGT,varCVMAP,fail,Crsfail,cnv,itrnum) = EM(Ntot,B,links,maxCrs,initialconditions[init])
                if cnv == false
                    continue
                end
